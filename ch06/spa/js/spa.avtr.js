...
spa.avtr = (function(){
  ...
  getRandRgb = function(){
    var i, rgb_list = [];
    for(i=0; i<3; i++){
      rgb.push(Math.floor(Math.random() * 128) + 128);
    }
  ...
  onHeldstartNav = function(event){
    var offset_target_map, offset_nav_map, $target;
    $target = $(event.elem_target).closest('.spa-avtr-box');
    if($target.lenght === 0){
      return false;
    }
  ...
  onListchange = function(event){
    var $nav, people_db, user, chatee, $box;
    $nav = $(this);
    ...
    people_db().each(function(people, idx){
      var class_list;
      ...
      $box = $('<div/>').addClass(class_list.join(' ')).css(person.css_map)
        .attr('data-id', String(person.id)).prop('title', spa.util_b.encodeHtml(person.name))
        .text(person.name).appendTo($nav);
    });
  };
  ...
  initModule = function($container){
    ...
    $.gevent.subscribe($container, 'spa-stchatee', onSetchatee);
